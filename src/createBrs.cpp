...
void topoana::createBrs(unsigned int size, string NM1, string NM2, string NM3, string NM4, string NM5, vector<string> vNm, vector<int> vICc, TTree * tr, unsigned int nMax, int * nq, int * iq, int * iCcq, int * nCcq, int * iqCc, int * nTotq)
{
  char nm1[50],nm2[50];
      ...
      if(m_ccSwitch==true)
        {
          ...
          if(vICc[i]==0)
          ...
          else
            {
              ...
              sprintf(specifier1, "nTot%s", nm1);
              sprintf(specifier2, "nTot%s/I", nm1);
              tr->Branch(specifier1, &nTotq[i], specifier2);
            }
        }
      else
        {
...
void topoana::createBrs(unsigned int size, string NM, vector<string> vNm, vector<int> vICc, TTree * tr, int * nq, int * nCcq, int * nTotq)
{
  int aICc[vICc.size()];
  for(unsigned int i=0;i<vICc.size();i++) aICc[i]=vICc[i];
      ...
      if(m_ccSwitch==true)
        {
          ...
          sprintf(specifier1, "nTot%s", NM.c_str());
          sprintf(specifier2, "nTot%s[%d]/I", NM.c_str(), int(size));
          tr->Branch(specifier1, &nTotq, specifier2);
        }
      else
        {
          sprintf(specifier1, "n%s", NM.c_str());
          ...
          if(m_ccSwitch==true)
            {
              ...
              tr->Branch(specifier1, &nCcq[i], specifier2);
              sprintf(specifier1, "nTot%s", nm);
              sprintf(specifier2, "nTot%s/I", nm);
              tr->Branch(specifier1, &nTotq[i], specifier2);
            }
          else
            {
